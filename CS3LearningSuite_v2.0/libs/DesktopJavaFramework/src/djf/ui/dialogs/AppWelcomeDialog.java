...
import static djf.AppPropertyType.HAS_TITLE_LABEL;
...
public class AppWelcomeDialog extends Stage {
    ...
    public AppWelcomeDialog(AppTemplate app) {
        ...
        labeled = props.isTrue(HAS_TITLE_LABEL);
        if(labeled) {
            String title = props.getProperty(WELCOME_DIALOG_TITLE);
        ...
        if(labeled) {
            titleLabel.getStyleClass().add(CLASS_DJF_TITLE_LABEL);
        }
